/*
    ...
    流程：
        ...
        5.资源释放。
    场景：程序一旦启动，部分参数是不可以修改的。
        比如：底盘驱动包，会设置通信的波特率、车轮直径、轮间距、电机编码器分辨率等。
        一旦驱动启动，这些参数原则上，都不可以被修改（只读模式），因为一旦修改，可能
        会导致一些运行问题。
    问题：怎么设置参数的只读模式？
