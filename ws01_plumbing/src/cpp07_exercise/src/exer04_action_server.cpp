...
private:
    ...
    rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const Nav::Goal> goal){
        (void)uuid;
        // (void)goal;
        // 取出目标中的 x y 坐标，分别判断是否超出了 [0,11.08]取值，如果超出，则认为非法，否则合法
    ...
    void execute(std::shared_ptr<rclcpp_action::ServerGoalHandle<Nav>> goal_handle){
        ...
        while (true)
        {
            ...
            // 2.发布乌龟运动指令
            // 根据剩余距离计算速度指令并发布
            float scale = 0.5;  // 速度比例系数
            //为了想让乌龟距离目标远时，速度快一些，距离近时，速度慢一些，所以把剩余距离作为乌龟的线速度，但是要乘以一个比例系数，防止速度过快
