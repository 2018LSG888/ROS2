/*
    ...
    BUB处理：
        BUG说明：客户端发送了请求，但是还没有执行完毕， 客户端按下 ctr + c , 客户端终止了， 但是服务端还在执行。
        预期结果: 客户端按下 ctrl + c 终止后，服务端也应该终止。
        BUB原因: 客户端只有发送取消请求，才能让服务端结束任务。
        BUB解决：
                在客户端按下 ctrl + c 后， 发送取消请求。
                关键点1：ctrl + c 按下后，会出发哪些执行代码？-----
                        rclcpp::ok() 返回 False
                        也会执行析构函数
                关键点2：取消请求的API是什么？----- client->async_cancel_all_goals();
        
        进   阶：
                取消任务更多的是由业务决定的，比如：导航的车辆陷入绝境时
*/
// 1.包含头文件；
    ...
    */
    void goal_response_callback(rclcpp_action::ClientGoalHandle<Progress>::SharedPtr goal_handle){
        if(!goal_handle){
            ...
            // 添加子线程
            std::thread(std::bind(&ProgressActionClient::cancel_goal,this)).detach();
        }
        
    }

    void cancel_goal(){
        // 判断 rclcpp::ok 是否返回true
        while(rclcpp::ok()){} // 
        client_->async_cancel_all_goals();
    }
